...
import { deleteReward } from '../../../hooks/queries/reward';
import  toast  from 'react-hot-toast';

export default function DeleteReward({isOpen, setIsOpen, closeModal, refatcher, reward}) {


    const onSubmit= (data) => {
        deleteRewardMutate(data)
    }
    function closeModal() {
        setIsOpen(false)
    }
    ...
    const {
        mutate: deleteRewardMutate,
        isLoading,
    } = useMutation(deleteReward, {
        onSuccess: (data) => {
            toast.success(data, {
                position: 'top-right',
            });
                ...
                <div className="inline-block w-full max-w-2xl pb-4 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-[3px]">
                    ...
                    <div className="flex items-center bg-teal-700 text-white py-4 px-4 mb-6 font-medium text-lg text-left rounded-t-[3px]">
                        ...
                        <div className='px-6'>
                                ...
                                <h2 className="text-lg font-semibold pb-5">Delete this Reward </h2>
                                <p className="text-gray-500 text-sm">
                                    Are you sure you want to delete this Reward? All of your data will
                                    ...
                                    <div className='flex justify-end mt-4'>
                                        ...
                                        {isLoading ? (
                                            ...
                                            <button onClick={() => onSubmit(reward)} className=' cursor-pointer bg-red-500 text-white font-normal px-4 py-1 rounded-md' > Delete </button>
                                        )}
                                        </div>
